...
public class PdfExporter : MonoBehaviour
{
    ...
    [Header("Settings")]
    [SerializeField] private int linesPerPage = 45;                       // tweak if you change font size
    ...
    public void Download()
    {
        if (reportText == null)
        {
            Debug.LogWarning("PdfExporter: <reportText> reference missing on " + name + ". Attach the component and set it in the Inspector.");
            return;
        }
        ...
        string[] allLines = plain.Split(new[] { '\n' }, StringSplitOptions.None);

        // 2. Paginate – chunk into X lines per page
        List<string[]> pages = new();
        for (int i = 0; i < allLines.Length; i += linesPerPage)
        {
            int cnt  = Mathf.Min(linesPerPage, allLines.Length - i);
            string[] slice = new string[cnt];
            Array.Copy(allLines, i, slice, 0, cnt);
            pages.Add(slice);
        }
    ...
    private static byte[] BuildPdf(List<string[]> pages)
    {
        ...
        for (int i = 0; i < pages.Count; i++)
        {
            ...
            sb.Append("BT /F1 10 Tf 40 ").Append(PAGE_H - 40).Append(" Td\n").Append("12 TL\n");   
